class AppsController < ApplicationController
  ...
  def index
    ...
    attachments = Event.where(:type => "Desman.Application", :subtype => "Info").where("attachment IS NOT NULL").having('timestamp = MAX(timestamp)').group(:app, :timestamp, :id).pluck(:app, :id, :attachment, :payload)
