class AppsController < ApplicationController
  ...
  def index
    ...
    attachments = Event.where(:app => @apps, :type => "Application", :subtype => "Info").where("attachment IS NOT NULL").group(:app).having('timestamp = MAX(timestamp)').pluck(:app, :id, :attachment, :payload)
