class UsersController < ApplicationController
  ...
  def index
    ...
    attachments = Event.where(:user => @users, :type => "Desman.Device", :subtype => "User").group(:user, :timestamp).having('timestamp = MAX(timestamp)').pluck(:user, :id, :attachment, :payload)
